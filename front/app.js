...
// Version de l'application - mettre à jour à chaque déploiement
...
/**
 * Vérifie la version et vide le cache si nécessaire
 */
async function checkVersionAndClearCache() {
  ...
  if (storedVersion !== currentVersion) {
    ...
    if (window.toastManager) {
      window.toastManager.info('Application mise à jour', 'Le cache a été vidé automatiquement.');
    }
...
async function clearApplicationCache() {
  ...
  try {
    ...
    // Vider le cache de l'application via DataManager
    if (window.dataManager && typeof window.dataManager.clearApplicationCache === 'function') {
      await window.dataManager.clearApplicationCache();
    } else {
      // Fallback si dataManager n'est pas encore disponible
      localStorage.removeItem('birthdayReminderData');
      console.log('Fallback: localStorage cleared');
    }
    
    // Vider d'autres caches spécifiques à l'application
    const cacheKeys = [
      'app-theme',
    ...
    // Forcer le rechargement des ressources CSS/JS
    forceReloadAssets();
...
/**
 * Force le rechargement des ressources CSS/JS
 */
function forceReloadAssets() {
  // Ajouter un timestamp aux ressources pour forcer le rechargement
  ...
  cssLinks.forEach(link => {
    if (link.href.includes('styles.css')) {
      const newHref = link.href.split('?')[0] + '?v=' + timestamp;
      link.href = newHref;
    }
  });
  
  // Note: Pour les fichiers JS, un rechargement de page est nécessaire
  // car ils sont déjà chargés en mémoire
}

/**
 * Fonction utilitaire pour vider le cache manuellement
 * Usage dans la console : clearCacheManually()
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Vérifier la version et vider le cache si nécessaire
